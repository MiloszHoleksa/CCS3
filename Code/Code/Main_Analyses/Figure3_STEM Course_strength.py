...
nSubj = D3.shape[2]
Result = np.zeros(shape=(D3.shape[2],6))
for m in range(D3.shape[2]):
    subj_id_test = m
    D_test = D3[:, :, subj_id_test]
    D_P_test = D_P[:, subj_id_test]
    D_X_test = D_X[:, subj_id_test]
    D_Y_test = D_Y[:, subj_id_test]
    #testbounds = np.array([540, 1110, 1670, 2500, 2910, 3860, 4620, 6850])  # high
    #testbounds = np.array([320, 2670, 2990, 3520, 4230, 5530, 6620, 7570])  # low
    testbounds = np.array([680, 2140, 3450, 5070, 5380, 6490, 7360, 7430])  # mid
    a = 0
    test_1 = testbounds
    while test_1[0] <= 100:
        a = 1
    ...
    Pprebou_ = np.zeros(len(test_))
    Pboundary_ = np.zeros(len(test_))
    Ppostbou_ = np.zeros(len(test_))
    for i in range(len(test_)):
        Pprebou_[i] = test_[i] - 50
        Pboundary_[i] = test_[i]
        Ppostbou_[i] = test_[i] + 50
    PD_ = np.concatenate(([Pprebou_], [Pboundary_], [Ppostbou_]))
    Pprebou_ = list(map(int, Pprebou_))
    Pboundary_ = list(map(int, Pboundary_))
    Ppostbou_ = list(map(int, Ppostbou_))
    Ppre_ = np.zeros(len(test_))
    Pbou_ = np.zeros(len(test_))
    Ppost_ = np.zeros(len(test_))
    for a in range(len(test_)):
        Ppre_[a] = np.mean(np.corrcoef(D_P_test[(Pprebou_[a] - 50):Pprebou_[a]],
                                        D_P_test[Pprebou_[a]:(Pprebou_[a] + 50)]))
        Pbou_[a] = np.mean(np.corrcoef(D_P_test[(Pboundary_[a] - 50):Pboundary_[a]],
                                        D_P_test[Pboundary_[a]:(Pboundary_[a] + 50)]))
        Ppost_[a] = np.mean(np.corrcoef(D_P_test[(Ppostbou_[a] - 50):Ppostbou_[a]],
                                        D_P_test[Ppostbou_[a]:(Ppostbou_[a] + 50)]))
    Result[m][0]  = np.mean(Ppre_)
    Result[m][1]  = np.mean(Pbou_)
    Result[m][2]  = np.mean(Ppost_)
    ################################################################
    Xprebou_ = np.zeros(len(test_))
    Xboundary_ = np.zeros(len(test_))
    Xpostbou_ = np.zeros(len(test_))
    for i in range(len(test_)):
        Xprebou_[i] = test_[i] - 50
        Xboundary_[i] = test_[i]
        Xpostbou_[i] = test_[i] + 50
    D_ = np.concatenate(([Xprebou_], [Xboundary_], [Xpostbou_]))
    Xprebou_ = list(map(int, Xprebou_))
    Xboundary_ = list(map(int, Xboundary_))
    Xpostbou_ = list(map(int, Xpostbou_))
    Xpre_ = np.zeros(len(test_))
    Xbou_ = np.zeros(len(test_))
    Xpost_ = np.zeros(len(test_))
    for a in range(len(test_)):
        Xpre_[a] = np.mean(np.corrcoef(D_X_test[(Xprebou_[a] - 50):Xprebou_[a]],
                                        D_X_test[Xprebou_[a]:(Xprebou_[a] + 50)]))
        Xbou_[a] = np.mean(np.corrcoef(D_X_test[(Xboundary_[a] - 50):Xboundary_[a]],
                                        D_X_test[Xboundary_[a]:(Xboundary_[a] + 50)]))
        Xpost_[a] = np.mean(np.corrcoef(D_X_test[(Xpostbou_[a] - 50):Xpostbou_[a]],
                                        D_X_test[Xpostbou_[a]:(Xpostbou_[a] + 50)]))
    Xallpre_ = np.mean(Xpre_)
    Xallbou_ = np.mean(Xbou_)
    Xallpost_ = np.mean(Xpost_)
    ##############################################################
    Yprebou_ = np.zeros(len(test_))
    Yboundary_ = np.zeros(len(test_))
    Ypostbou_ = np.zeros(len(test_))
    for i in range(len(test_)):
        Yprebou_[i] = test_[i] - 50
        Yboundary_[i] = test_[i]
        Ypostbou_[i] = test_[i] + 50
    D_ = np.concatenate(([Yprebou_], [Yboundary_], [Ypostbou_]))
    Yprebou_ = list(map(int, Yprebou_))
    Yboundary_ = list(map(int, Yboundary_))
    Ypostbou_ = list(map(int, Ypostbou_))
    Ypre_ = np.zeros(len(test_))
    Ybou_ = np.zeros(len(test_))
    Ypost_ = np.zeros(len(test_))
    for a in range(len(test_)):
        Ypre_[a] = np.mean(np.corrcoef(D_Y_test[(Yprebou_[a] - 50):Yprebou_[a]],
                                        D_Y_test[Yprebou_[a]:(Yprebou_[a] + 50)]))
        Ybou_[a] = np.mean(np.corrcoef(D_Y_test[(Yboundary_[a] - 50):Yboundary_[a]],
                                        D_Y_test[Yboundary_[a]:(Yboundary_[a] + 50)]))
        Ypost_[a] = np.mean(np.corrcoef(D_Y_test[(Ypostbou_[a] - 50):Ypostbou_[a]],
                                        D_Y_test[Ypostbou_[a]:(Ypostbou_[a] + 50)]))
    Yallpre_ = np.mean(Ypre_)
    Yallbou_ = np.mean(Ybou_)
    Yallpost_ = np.mean(Ypost_)
    Result[m][3]  = (Xallpre_ + Yallpre_ + Result[m][0]) / 3
    Result[m][4]  = (Xallbou_ + Yallbou_ + Result[m][1]) / 3
    Result[m][5]  = (Xallpost_ + Yallpost_ + Result[m][2]) / 3
    print(Result)



workbook = xw.Workbook('similarity5s strength mid.xls', {'nan_inf_to_errors': True})  # 创建工作簿
...
for j in range(len(data)):
    insertData = [data[j][0], data[j][1],data[j][2], data[j][3],data[j][4], data[j][5]]
    row = 'A' + str(i)
