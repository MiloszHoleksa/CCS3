...
nSubj = D3.shape[2]
nTRs = D3.shape[1]
Result_Correlation = np.zeros(D3.shape[2])
Result_p=np.zeros(D3.shape[2])
for a in range(D3.shape[2]):
    subj_id_test = a
    subj_id_train = [
        subj_id for subj_id in range(nSubj)
        if subj_id not in [subj_id_test]
    ]
#    subj_id_train=random.sample(range(nSubj),19)
    ...
    #testbounds = testbounds[1:]
    f, ax = plt.subplots(1, 1, figsize=(12, 4))
    predtest_seg = test_segments
    ax.imshow(predtest_seg.T, aspect='auto', cmap='viridis')
    ...
    KZ = np.zeros(len(testbounds) + 1)
    for i in range(len(bounds_aug) - 1):
        KZ_ma = DD[bounds_aug[i]:bounds_aug[i + 1], bounds_aug[i]:bounds_aug[i + 1]]
        KZ[i] = np.mean(KZ_ma)
        KZ_mean = np.mean(KZ)
        #print(KZ_mean)

#    print('##################################')
    print('Subj ' + str(a+1) + ' Correlation = ' + str(KZ_mean))
    print(KZ)
    ...
    print('Subj ' + str(a+1) + 'p value = ' + str(norm.sf((match[0]-match[1:].mean())/match[1:].std())))
