...
A = np.array(
    [ data1.T, data2.T, data3.T, data7.T, data9.T, data12.T, data13.T, data16.T, data17.T, data18.T, data19.T, data23.T,
     data26.T, data27.T, data30.T, data32.T, data35.T, data38.T, data41.T, data45.T, data55.T, data63.T,
...
B = np.array(
    [data33.T, data34.T, data39.T, data42.T, data43.T, data47.T, data48.T, data49.T, data50.T, data51.T, data52.T,
     data53.T, data58.T, data60.T, data62.T, data67.T, data70.T, data72.T, data73.T, data75.T,
...
nTRs = D1.shape[1]
D1_means = np.mean(D1, axis=2)
D2_means = np.mean(D2, axis=2)
D3_means = np.mean(D3, axis=2)

k=20

nSubj1 = D1.shape[2]
Result1 = np.zeros(D1.shape[2])
Resultmath1 = np.zeros(shape=(D1.shape[2],1001))
for m in range(D1.shape[2]):
    subj_id_test = m
    D_test = D1[:, :, subj_id_test]
    D_test = stats.zscore(D_test, axis=1, ddof=1)
    testbounds = np.array([340, 1530, 2300, 3600, 4790, 5290, 7000, 7540, 8380, 8610,
                             8800, 9290, 9650, 10260, 10600, 11130, 11810, 12450, 12870])#human boundary
    DD = np.corrcoef(D_test.T)#生成时间序列相关矩阵
    bounds_aug = np.concatenate(([0], testbounds, [nTRs]))
    event_counts = np.diff(np.concatenate(([0], testbounds, [nTRs])))#变成'时间矩阵块'，为后面打乱重排做准备
    nPerm = 1000
    SC = np.zeros(len(testbounds) + 1)
    match = np.zeros(nPerm + 1)
    for p in range(nPerm + 1):#1001 1是实际值，后面1000是打乱重排的值
        for i in range(len(bounds_aug) - 1):
            SC_ma = DD[bounds_aug[i]:bounds_aug[i + 1], bounds_aug[i]:bounds_aug[i + 1]]#每一个时间矩阵块
            SC[i] = np.mean(SC_ma)#每一个块的平均一致性
        match[p] = np.mean(SC)
        perm_counts = np.random.permutation(event_counts)#第二次开始打乱
        perm_bounds = np.cumsum(perm_counts)[:-1]
        bounds_aug = np.concatenate(([0], perm_bounds, [nTRs]))
    Resultmath1[m,:]=match
    Result1[m]=str(norm.sf((match[0]-match[1:].mean())/match[1:].std()))
    print('Subj ' + str(m+1) + 'p value = ' + str(norm.sf((match[0]-match[1:].mean())/match[1:].std())))

nSubj2 = D2.shape[2]
Result2 = np.zeros(D2.shape[2])
Resultmath2 = np.zeros(shape=(D2.shape[2],1001))
for n in range(D2.shape[2]):
    subj_id_test = n
    D_test = D2[:, :, subj_id_test]
    D_test = stats.zscore(D_test, axis=1, ddof=1)
    testbounds = np.array([340, 1530, 2300, 3600, 4790, 5290, 7000, 7540, 8380, 8610,
                             8800, 9290, 9650, 10260, 10600, 11130, 11810, 12450, 12870])
    DD = np.corrcoef(D_test.T)
    bounds_aug = np.concatenate(([0], testbounds, [nTRs]))
    ...
    for p in range(nPerm + 1):
        for i in range(len(bounds_aug) - 1):
            SC_ma = DD[bounds_aug[i]:bounds_aug[i + 1], bounds_aug[i]:bounds_aug[i + 1]]
            SC[i] = np.mean(SC_ma)
        match[p] = np.mean(SC)
        ...
        bounds_aug = np.concatenate(([0], perm_bounds, [nTRs]))
    Resultmath2[n,:]=match
    Result2[n]=str(norm.sf((match[0]-match[1:].mean())/match[1:].std()))
    print('Subj ' + str(n+1) + 'p value = ' + str(norm.sf((match[0]-match[1:].mean())/match[1:].std())))

nSubj3 = D3.shape[2]
Result3 = np.zeros(D3.shape[2])
Resultmath3 = np.zeros(shape=(D3.shape[2],1001))
for o in range(D3.shape[2]):
    subj_id_test = o
    ...
    for p in range(nPerm + 1):
        ...
        bounds_aug = np.concatenate(([0], perm_bounds, [nTRs]))
    Resultmath3[o,:]=match
    Result3[o]=str(norm.sf((match[0]-match[1:].mean())/match[1:].std()))
    print('Subj ' + str(o+1) + 'p value = ' + str(norm.sf((match[0]-match[1:].mean())/match[1:].std())))

nSubj4 = D.shape[2]
Result4 = np.zeros(D.shape[2])
Resultmath4 = np.zeros(shape=(D.shape[2],1001))
for q in range(D.shape[2]):
    subj_id_test = q
    ...
    for p in range(nPerm + 1):
        ...
        bounds_aug = np.concatenate(([0], perm_bounds, [nTRs]))
    Resultmath4[q,:]=match
    Result4[q]=str(norm.sf((match[0]-match[1:].mean())/match[1:].std()))
    print('Subj ' + str(q+1) + 'p value = ' + str(norm.sf((match[0]-match[1:].mean())/match[1:].std())))

Resultmath=np.concatenate((Resultmath1, Resultmath2, Resultmath3,Resultmath4), axis=0)
...
plt.ylabel('Commercial Movie Human Correlation value',size=15)
plt.show()