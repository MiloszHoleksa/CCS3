...
for m in range(D1.shape[2]):
    subj_id_test = m
    subj_id_train = [
        subj_id for subj_id in range(nSubj1)
        if subj_id not in [subj_id_test]
    ]

    D_train = D1[:, :, subj_id_train]
...
for n in range(D2.shape[2]):
    subj_id_test = n
    subj_id_train = [
        subj_id for subj_id in range(nSubj2)
        if subj_id not in [subj_id_test]
    ]

    D_train = D2[:, :, subj_id_train]
...
for o in range(D3.shape[2]):
    subj_id_test = o
    subj_id_train = [
        subj_id for subj_id in range(nSubj3)
        if subj_id not in [subj_id_test]
    ]
...
for q in range(D.shape[2]):
    subj_id_test = q
    subj_id_train = [
        subj_id for subj_id in range(nSubj4)
        if subj_id not in [subj_id_test]
    ]
...
plt.ylabel('Commercial Movie HMM Correlation value',size=15)
plt.show()
