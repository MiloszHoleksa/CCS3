...
#2->23 3->32
nSubj = D2.shape[2]
Result = np.zeros(shape=(D2.shape[2],5))
for m in range(D2.shape[2]):
    subj_id_test = m
    subj_id_train = [
    ...
    D_P_test = D2_P[:, subj_id_test]
    D_X_test = D2_X[:, subj_id_test]
    D_Y_test = D2_Y[:, subj_id_test]
    ...
    for i in range(len(test_)):
        pre2bou_[i] = math.sqrt((D_X_test[(test_[i] - 25)] - D_X_test[(test_[i] - 15)]) ** 2 +
                                (D_Y_test[(test_[i] - 25)] - D_Y_test[(test_[i] - 15)]) ** 2)
        pre1bou_[i] = math.sqrt((D_X_test[(test_[i] - 15)] - D_X_test[(test_[i] - 5)]) ** 2 +
                                (D_Y_test[(test_[i] - 15)] - D_Y_test[(test_[i] - 5)]) ** 2)
        boundary_[i] = math.sqrt((D_X_test[(test_[i] - 5)] - D_X_test[(test_[i] + 5)]) ** 2 +
                                 (D_Y_test[(test_[i] - 5)] - D_Y_test[(test_[i] + 5)]) ** 2)
        post1bou_[i] = math.sqrt((D_X_test[(test_[i] + 5)] - D_X_test[(test_[i] + 15)]) ** 2 +
                                 (D_Y_test[(test_[i] + 5)] - D_Y_test[(test_[i] + 15)]) ** 2)
        post2bou_[i] = math.sqrt((D_X_test[(test_[i] + 15)] - D_X_test[(test_[i] + 25)]) ** 2 +
                                 (D_Y_test[(test_[i] + 15)] - D_Y_test[(test_[i] + 25)]) ** 2)
...
workbook = xw.Workbook('speed Commercial movie2.xls', {'nan_inf_to_errors': True})  # 创建工作簿
