...
D = D.transpose(1, 2, 0)
D_all = np.concatenate((D1, D2, D3,D), axis=2)
D_all23=np.concatenate(( D2, D3), axis=2)
D1_P = D1[2, :, :]
D2_P = D2[2, :, :]
D3_P = D3[2, :, :]
D_P = D[2, :, :]
...
for m in range(D3.shape[2]):
    ...
    D_P_test = D3_P[:, subj_id_test]
    D_X_test = D3_X[:, subj_id_test]
    D_Y_test = D3_Y[:, subj_id_test]
...
workbook = xw.Workbook('similarity5s Commercial movie3.xls', {'nan_inf_to_errors': True})  # 创建工作簿
